...
public class ConversationSqlManager extends AbstractSQLManager {
    ...
    public static Cursor getConversationCursorByType(int type) {
        try {
            ...
            String sql = "SELECT unreadCount, im_thread.type, sendStatus, dateTime, sessionId, text, username ,name ,im_thread.contactid ,isnotice\n" +
                    ...
                    "      WHERE im_thread.conv_type = " + type + " order by dateTime desc;";
            return getInstance().sqliteDB().rawQuery(sql, null);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static long insertSessionRecord(ECMessage msg) {
        ...
        int iType = 0;
        String udata = msg.getUserData();
        if (udata != null) {
            try {
                JSONObject jo1 = new JSONObject(udata);
                String typew = (String)jo1.get("type");
                if ("0".equals(typew)) {
                    JSONObject jo2 = (JSONObject)jo1.get("to");
                    String id = (String)jo2.get("id");
                    String typec = (String)jo2.get("type");
        ...
        try {
            ...
            values.put(IThreadColumn.CONVERSATION_TYPE, iType);
