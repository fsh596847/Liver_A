...
public class PagerSlidingTabStrip extends HorizontalScrollView implements
	...
	/**
	 * 调整views集合中的View，让�?有View的宽度加起来正好等于parentViewWidth
	 * 
	 * @param views
	 *            子View集合
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (tabViewGroup != null) {
			...
			currentPosition = viewPager != null ? viewPager.getCurrentItem()
					: 0;
			if (!disableViewPager) {
				scrollToChild(currentPosition, 0); // 移动滑块到指定位�?
				selectedTab(currentPosition); // 选中指定位置的TAB
			}

			// 给每�?个tab设置点击事件，当点击的时候切换Pager
	...
	protected void onDraw(Canvas canvas) {
		...
		if (tabsLayout != null && tabsLayout.getChildCount() > 0
			...
			if (currentTab != null) {
				...
				slidingBlockDrawable.setBounds((int) slidingBlockLeft + 60, 0,
						(int) slidingBlockRight - 60, getHeight());
	 ...
	 *            true：当内容的宽度无法充满屏幕时，自动调整每�?个Item的宽度以充满屏幕
	 */
