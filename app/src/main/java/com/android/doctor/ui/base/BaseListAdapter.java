...
public class BaseListAdapter<T> extends BaseAdapter {
    ...
    public BaseListAdapter() {
        mInflater = LayoutInflater.from(AppContext.context());
    ...
    public T getItem(int position) {
        ...
        if (position < 0 || mDatas.size() <= position) return null;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (position == getCount() - 1&&hasFooterView()) {// 最后一条
            ...
            if (getState() == STATE_LOAD_MORE || getState() == STATE_NO_MORE
                ...
                this.mFooterView = (LinearLayout) mInflater.inflate(R.layout.list_cell_footer,
                        null);
                if (!loadMoreHasBg()) {
