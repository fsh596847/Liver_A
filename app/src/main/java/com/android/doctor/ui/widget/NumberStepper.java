...
public class NumberStepper extends RelativeLayout implements View.OnTouchListener,
    ...
    //按下后多少间隔触发快速改变模式
    private static final long STEPSPEEDCHANGEDURATION = 100;
    ...
    private void initViews(AttributeSet attrs) {
        ...
        LayoutInflater.from(getContext()).inflate(R.layout.view_number_stepper, this, true);
        ...
        int contentTextColor = getResources().getColor(R.color.appThemePrimary);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                stepTouch = true;
                postDelayed(updateRunnable, UPDATEDURATIONFAST);
                //非按钮则记录位置
                startX = event.getX();
