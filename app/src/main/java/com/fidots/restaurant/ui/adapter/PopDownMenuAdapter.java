...
public class PopDownMenuAdapter extends BaseAdapter {
    ...
    public PopDownMenuAdapter(Context context, List<String> list, int layout, boolean isGrid) {
    ...
    private void fillValue(int position, ViewHolder viewHolder) {
        ...
        if (checkItemPosition != -1) {
            if (checkItemPosition == position) {
                ...
                viewHolder.mText.setTextColor(context.getResources().getColor(R.color.divider_color));
                if (mIsGrid) {
                    viewHolder.mText.setBackgroundResource(R.drawable.item_unchecked_bg);
                } else {
